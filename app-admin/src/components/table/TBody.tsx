...
function TBodyInner<TData>({
  ...
  if (loading) return <TBodyLoading colSpan={colSpan} />; // 로딩 + 아무 데이터 없을 때
  if (!loading && data.length === 0) return <TBodyEmpty emptyText={emptyText} colSpan={colSpan} />; // 빈 상태
  return (
    <tbody>
      {rows.map((r, i) => {
        ...
        return (
          <tr
            ...
            className={clsx(
              href ? 'cursor-pointer rounded' : '',
              'text-center',
              // 줄무늬
              'odd:bg-stone-950 even:bg-black/40',
              // 호버/포커스
              'hover:bg-zinc-900 focus-visible:outline focus-visible:outline-indigo-500',
            )}
          >
            {r.getVisibleCells().map((cell) => (
              <td
                key={cell.id}
                className="max-w-[240px] overflow-hidden px-3 py-2 text-ellipsis whitespace-nowrap"
              >
